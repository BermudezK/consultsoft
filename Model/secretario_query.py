...
def modificar_secretario(nombre,apellido,telefono,dni):
	try:        
		cursor = mydb.cursor()         
		consulta = (f"update personal SET nombre = '{nombre}', apellido = '{apellido}', telefono = {telefono} where personal_DNI = {dni}")       
		cursor.execute(consulta)      
		mydb.commit()   
	except Error as e:     
...
def modificar_usuario(nombreusuario,contraseña,userID):
	try:        
		cursor = mydb.cursor()         
		consulta = (f"update usuario SET userName = '{nombreusuario}', password = '{contraseña}'  where usuario_ID = {userID}")       
		cursor.execute(consulta)      
		mydb.commit()     
	except Error as e :     
		print ("Error while connecting to MySQL", e)  
